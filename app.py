...
# ==== BEGIN APPENDED: Print / Export (New) — safe addon ====
# This provides a standalone section at the end of the app with:
#  - Browser Print
#  - Download HTML (always works; you can print to PDF from browser)
#  - Download PDF (if "reportlab" is installed in requirements)
# It uses your existing `data` dict and Incidents/child tables.
try:
    ...
    def _ensure_cols_addon(df: _pd.DataFrame, cols: List[str]):
        if df is None:
            df = _pd.DataFrame()
        for c in cols:
            if c not in df.columns:
    ...
    def _pdf_bytes_addon(incident, ip_view, ia_view, times_row, incident_id):
        buf = _io.BytesIO()
        ...
        def _draw(lines, font=("Helvetica",10), leading=12):
            ...
            for line in (lines if isinstance(lines, list) else [lines]):
                ...
                y -= leading
                if y < 72:
                    c.showPage(); y = height - 0.75 * _inch
        ...
        _draw([""], leading=10); _draw(["Personnel on Scene"], font=("Helvetica-Bold",12), leading=14)
        _draw(["Name | Role | Hours | RespondedIn"])
        _draw(["-"*92])
        for row in _rows(ip_view, ["Name","Role","Hours","RespondedIn"]):
            _draw([" | ".join("" if v is None else str(v) for v in row)])

        _draw([""], leading=10); _draw(["Apparatus on Scene"], font=("Helvetica-Bold",12), leading=14)
        _draw(["Unit | UnitType | Role | Actions"])
        _draw(["-"*92])
        for row in _rows(ia_view, ["Unit","UnitType","Role","Actions"]):
            _draw([" | ".join("" if v is None else str(v) for v in row)])
    ...
    _st.markdown("---")
    _st.header("🖨️ Print / Export (New)")

    # Discover variables from your app (do not crash if absent)
    try:
        _PRIMARY_KEY_DISC = PRIMARY_KEY
    except NameError:
        _PRIMARY_KEY_DISC = "IncidentNumber"
    try:
        _data_disc = data
    except NameError:
        _data_disc = {}

    if not isinstance(_data_disc, dict) or "Incidents" not in _data_disc or _data_disc["Incidents"].empty:
        _st.warning("Incidents table not available here. Load the workbook first (or create a report), then scroll back here.")
    else:
        base = _data_disc["Incidents"].copy()
        # optional status filter
        status = _st.selectbox("Filter by Status", options=["","Approved","Submitted","Draft","Rejected"], index=0)
        if status:
            base = base[base["Status"].astype(str) == status]
        _st.dataframe(base, use_container_width=True, hide_index=True, key="grid_print_new_base")

        options = base[_PRIMARY_KEY_DISC].dropna().astype(str).tolist() if _PRIMARY_KEY_DISC in base.columns else []
        sel_new = _st.selectbox("Pick an Incident to print/export", options=options, index=None, placeholder="Choose...", key="pick_print_new")

        if sel_new:
            rec = base[base[_PRIMARY_KEY_DISC].astype(str) == str(sel_new)].iloc[0].to_dict()
            ip = _ensure_cols_addon(_data_disc.get("Incident_Personnel", _pd.DataFrame()),
                                    ["IncidentNumber","Name","Role","Hours","RespondedIn"])
            ia = _ensure_cols_addon(_data_disc.get("Incident_Apparatus", _pd.DataFrame()),
                                    ["IncidentNumber","Unit","UnitType","Role","Actions"])
            tdf = _ensure_cols_addon(_data_disc.get("Incident_Times", _pd.DataFrame()),
                                    ["IncidentNumber","Alarm","Enroute","Arrival","Clear"])
            ip_view = ip[ip[_PRIMARY_KEY_DISC].astype(str) == str(sel_new)]
            ia_view = ia[ia[_PRIMARY_KEY_DISC].astype(str) == str(sel_new)]
            times_row = {}
            m = tdf[_PRIMARY_KEY_DISC].astype(str) == str(sel_new)
            if m.any():
                times_row = tdf[m].iloc[0].to_dict()

            # Browser print
            c1, c2, c3 = _st.columns(3)
            if c1.button("🖨️ Print Page", key=f"btn_print_new_{sel_new}"):
                _components.html("<script>window.print()</script>", height=0, width=0)

            # HTML export (always available)
            import html as _html
            def esc(x): return _html.escape("" if x is None else str(x))
            html_report = f"""
<h2>Incident #{esc(sel_new)}</h2>
...
<b>Times:</b> Alarm {esc(times_row.get('Alarm',''))} | Enroute {esc(times_row.get('Enroute',''))} | Arrival {esc(times_row.get('Arrival',''))} | Clear {esc(times_row.get('Clear',''))}<br><br>
<h3>Narrative</h3>
<div style="white-space: pre-wrap;">{esc(rec.get('Narrative',''))}</div>
<br>
<h3>Personnel on Scene</h3>
{ip_view.to_html(index=False)}
<br>
<h3>Apparatus on Scene</h3>
{ia_view.to_html(index=False)}
"""
            c2.download_button("⬇️ Download HTML", html_report, file_name=f"Incident_{sel_new}.html", mime="text/html", key=f"dl_html_new_{sel_new}")

            # PDF export (if reportlab available)
            if _PDF_OK_ADDON and c3.button("📄 Download PDF", key=f"btn_pdf_new_{sel_new}"):
                try:
                    pdf = _pdf_bytes_addon(rec, ip_view, ia_view, times_row, str(sel_new))
                    _st.download_button("Save PDF", pdf, file_name=f"Incident_{sel_new}.pdf",
                                        mime="application/pdf", key=f"dl_pdf_new_{sel_new}")
                except Exception as e:
                    _st.error(f"PDF failed: {e}")
        else:
            _st.info("Select an incident above to show Print/Export controls.")
except Exception as _addon_e:
    try:
        import streamlit as _st
        _st.error(f"Print / Export (New) failed to load: {_addon_e}")
    except Exception:
        pass
# ==== END APPENDED: Print / Export (New) — safe addon ====
