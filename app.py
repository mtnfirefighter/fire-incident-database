...
# Display-only roster identifiers for convenience (does not alter saved data)
try:
    _overlay_df = render_personnel_roster_overlay(data, inc_num, PRIMARY_KEY=PRIMARY_KEY)
    if not _overlay_df.empty:
        st.caption("Roster identifiers (display-only)")
        st.dataframe(_overlay_df, use_container_width=True, hide_index=True)
except Exception:
    pass
...
# === Helper appended by patch (display-only overlay in Write tab) ===
def render_personnel_roster_overlay(data, inc_value, PRIMARY_KEY='IncidentNumber'):
    import pandas as _pd
    # Pull incident personnel for this incident
    ip = data.get('Incident_Personnel', _pd.DataFrame()).copy()
    if ip is None or ip.empty:
        return _pd.DataFrame(columns=['Identifier','Name','Role','RespondedIn'])
    if PRIMARY_KEY in ip.columns:
        try:
            ip = ip[ip[PRIMARY_KEY].astype(str) == str(inc_value)].copy()
        except Exception:
            ip = ip.copy()
    # Ensure expected cols exist
    for c in ['PersonnelID','Name','Role','RespondedIn']:
        if c not in ip.columns:
            ip[c] = _pd.NA
    roster = data.get('Personnel', _pd.DataFrame()).copy()
    if roster is None:
        roster = _pd.DataFrame()
    # Normalize roster columns we want
    if not roster.empty:
        if 'PersonnelID' not in roster.columns:
            for alt in ('ID','MemberID'):
                if alt in roster.columns:
                    ...
                    break
        if 'Name' not in roster.columns and ('FirstName' in roster.columns or 'LastName' in roster.columns):
        ...
        if 'Callsign' not in roster.columns:
            for alt in ('CallSign','call_sign','RadioID','Badge','BadgeNumber'):
                if alt in roster.columns:
                    ...
                    break
        # Merge for display: bring PersonnelID/UnitNumber/Callsign by Name
        if 'Name' in ip.columns and 'Name' in roster.columns:
            enrich_cols = [c for c in ['PersonnelID','UnitNumber','Callsign'] if c in roster.columns]
            if enrich_cols:
                _r = roster[['Name'] + enrich_cols].drop_duplicates()
                ip = ip.merge(_r, on='Name', how='left', suffixes=('', '_roster'))
                for c in enrich_cols:
                    cr = f'{c}_roster'
                    if cr in ip.columns:
                        ip[c] = ip[c].where(~ip[c].isna(), ip[cr])
                        ip.drop(columns=[cr], inplace=True, errors='ignore')
    # Build Identifier column
    ident = _pd.Series(_pd.NA, index=ip.index, dtype='object')
    for c in ('PersonnelID','UnitNumber','Callsign'):
        if c in ip.columns and getattr(ip[c], 'notna', lambda: False)().any():
            ident = ip[c]
            break
    out = _pd.DataFrame({
        'Identifier': ident,
        'Name': ip.get('Name', _pd.Series([], dtype='object')),
        'Role': ip.get('Role', _pd.Series([], dtype='object')),
        'RespondedIn': ip.get('RespondedIn', _pd.Series([], dtype='object')),
    })
    return out
