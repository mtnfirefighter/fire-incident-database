...
with tabs[5]:
    ...
    if sel:
        ...
        # Resolve the selected incident row
        try:
        ...
        # Personnel/Apparatus for this incident
...
"""
        ...
        # Download HTML (works everywhere)
        c2.download_button("⬇️ Download HTML", html_report, file_name=f"Incident_{sel}.html", mime="text/html", key=f"print_tab_html_{sel}")

        # Optional PDF (needs reportlab in requirements.txt)
        if _PDF_OK and c3.button("📄 Download PDF", key=f"print_tab_pdf_{sel}"):
            try:
                ...
                buf.seek(0)
                st.download_button("Save PDF", data=buf, file_name=f"Incident_{sel}.pdf", mime="application/pdf", key=f"print_tab_pdf_dl_{sel}")
            except Exception as e:
