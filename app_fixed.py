...
CHILD_TABLES = {
    ...
    "Incident_Personnel": ["IncidentNumber","Name","Role","Hours","RespondedIn","PersonnelID","Notes"],
...
def _clean_token(s: str) -> str:
    # remove stray bracketed exports like "['cpt']"
    s = "" if s is None else str(s)
    s = s.replace("["," ").replace("]"," ").replace("'"," ").replace('"'," ")
    return " ".join(s.split())

def _norm(s: str) -> str:
    return " ".join(_clean_token(s).lower().split())

def _label_variants_from_row(row) -> list:
    first = _nz(row.get("FirstName"))
    last  = _nz(row.get("LastName"))
    rank  = _nz(row.get("Rank"))
    name  = _nz(row.get("Name"))
    full  = _nz(row.get("FullName"))
    combos = [
        f"{first} {last}",
        f"{last}, {first}",
        f"{rank} {first} {last}".strip(),
        f"{first} {last} {rank}".strip(),
        first, last, rank, name, full
    ]
    seen, out = set(), []
    for v in combos:
        v = _norm(v)
...
def _build_personnel_index(roster_df):
    # returns dict: normalized_label -> {"PersonnelID": "...", "DisplayName": "Rank First Last or Name"}
    idx = {}
    ...
    if "firstname" in cols and "FirstName" not in r.columns: r.rename(columns={cols["firstname"]:"FirstName"}, inplace=True)
    if "lastName".lower() in cols and "LastName" not in r.columns: r.rename(columns={cols["lastname"]:"LastName"}, inplace=True)
    if "fullname" in cols and "FullName" not in r.columns: r.rename(columns={cols["fullname"]:"FullName"}, inplace=True)
    if "personnelid" in cols and "PersonnelID" not in r.columns: r.rename(columns={cols["personnelid"]:"PersonnelID"}, inplace=True)

    for _, row in r.fillna("").iterrows():
        pid = _clean_token(row.get("PersonnelID"))
        # Build a nice display name
        disp = " ".join(p for p in [_clean_token(row.get("Rank")), _clean_token(row.get("FirstName")), _clean_token(row.get("LastName"))] if p) or _clean_token(row.get("Name")) or _clean_token(row.get("FullName"))
        if not disp:
            continue
        for lbl in _label_variants_from_row(row):
            idx[lbl] = {"PersonnelID": (pid or None), "DisplayName": disp}
    return idx
    ...
    with st.container(border=True):
        ...
        if cc[3].button("Add Selected Members", key="w_add_people_btn_auth"):
            if not inc_num or str(inc_num).strip() == "":
                ...
                # Build index from roster
                roster_idx = _build_personnel_index(people_df)
                new_rows = []
                for pick in picked_people:
                    lab = _norm(pick)
                    rec = roster_idx.get(lab, {"PersonnelID": None, "DisplayName": pick})
                    new_rows.append({
                        PRIMARY_KEY: inc_key,
                        "Name": rec.get("DisplayName") or pick,
                        "Role": role_default,
                        "Hours": hours_default,
