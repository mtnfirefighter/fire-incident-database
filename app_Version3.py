...
with tabs[0]:
    ...
    with st.container(border=True):
        ...
        inc_date = c2.date_input("IncidentDate", value=pd.to_datetime(defaults.get("IncidentDate")).date() if defaults.get("IncidentDate") is not None and str(defaults.get("IncidentDate")) != "Na[...]
        inc_time = c3.text_input("IncidentTime (HH:MM)", value=str(defaults.get("IncidentTime","")) if defaults else "", key="w_inc_time_auth")
        c4, c5, c6 = st.columns(3)
        inc_type = c4.selectbox("IncidentType", options=[""]+lookups.get("IncidentType", []), index=([""]+lookups.get("IncidentType", [])).index(str(defaults.get("IncidentType",""))) if defaults.[...]
        inc_prio = c5.selectbox("ResponsePriority", options=[""]+lookups.get("ResponsePriority", []), index=([""]+lookups.get("ResponsePriority", [])).index(str(defaults.get("ResponsePriority",""[...]
        inc_alarm = c6.selectbox("AlarmLevel", options=[""]+lookups.get("AlarmLevel", []), index=([""]+lookups.get("AlarmLevel", [])).index(str(defaults.get("AlarmLevel",""))) if defaults.get("Al[...]
    ...
    with st.container(border=True):
        ...
        if cc[3].button("Add Selected Members", key="w_add_people_btn_auth"):
            if not inc_num or str(inc_num).strip() == "":
                ...
                # ensure current incident-personnel df
                df = ensure_columns(data.get("Incident_Personnel", pd.DataFrame()), CHILD_TABLES["Incident_Personnel"])

                # Build new rows with PersonnelID lookups using the helper in patch_personnelid_assign_v4_3_2.py
                # This will attempt to match the selected display names to Personnel roster rows
                new_rows = ppa.build_personnel_rows(
                    incident_id=inc_key,
                    picks=picked_people,
                    default_role=role_default,
                    default_hours=hours_default,
                    responded_in=(responded_in_default or None),
                    notes=None,
                    data=data
                )

                if new_rows:
                    # Merge into Incident_Personnel (avoids creating exact duplicates)
                    data["Incident_Personnel"] = ppa.merge_into_incident_personnel(df, new_rows, primary_key=PRIMARY_KEY)
                    ...
                    st.warning("No members selected or names not matched to roster entries.")
        ...
        if can(user,"CanReview"):
            if c[0].button("Approve", key="btn_approve_queue_auth"):
                if not can(user,"CanApprove"):
                    ...
                    row = rec; row["Status"] = "Approved"; row["ReviewedBy"] = user.get("Username"); row["ReviewedAt"] = datetime.now().strftime("%Y-%m-%d %H:%M"); row["ReviewerComments"] = comme[...]
                    data["Incidents"] = upsert_row(data["Incidents"], row, key=PRIMARY_KEY)
                    if st.session_state.get("autosave", True): save_to_path(data, file_path)
                    st.success("Approved.")
            if c[1].button("Reject", key="btn_reject_queue_auth"):
                row = rec; row["Status"] = "Rejected"; row["ReviewedBy"] = user.get("Username"); row["ReviewedAt"] = datetime.now().strftime("%Y-%m-%d %H:%M"); row["ReviewerComments"] = comments [...]
                data["Incidents"] = upsert_row(data["Incidents"], row, key=PRIMARY_KEY)
                if st.session_state.get("autosave", True): save_to_path(data, file_path)
                st.warning("Rejected.")
